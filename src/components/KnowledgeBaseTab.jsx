...
import { BookOpen, Upload, Trash, Pencil } from './Icons';
...
// ── Mock data (pre-loaded from the Wolfpack flyer) ────────────────────────────
...
• Coaches have final say on weight class assignments — no disputes on the floor
• Parents are NOT permitted on the mat or in the coaching area during competition
• Arrive at weigh-ins 15 minutes early — missing the window forfeits the day
• Win or lose, shake hands and thank your opponent

Violations are handled at the coach's discretion and may affect future tournament eligibility.`,
    fileName: null,
...
A: Singlet, headgear, wrestling shoes, water bottle, healthy snacks, and good energy. No outside coolers at most venues.
...
A: Please leave it to the staff. Sideline coaching creates confusion and distracts your wrestler. Cheer loudly, coach never.

Q: When do I find out the schedule?
A: Sessions are divided by age group. Check the tournament flyer in the Knowledge Base for exact times — weigh-in is typically 1–1.5 hrs before mat time.
...
function AddEntrySheet({ onClose, onSave }) {
  const [step, setStep]         = useState('choose');   // 'choose' | 'type' | 'upload'
  ...
  const handleSave = () => {
    ...
    onSave({ title: title.trim(), category, content: content.trim(), fileName: null, createdAt: 'Just now' });
    onClose();
  };
  ...
  return (
    <div style={{
      ...
      <div style={{
        ...
        padding: '20px 16px 32px', maxWidth: 430, width: '100%', margin: '0 auto',
        ...
        {step === 'choose' && (
          <>
            ...
            <input value={title} onChange={e => setTitle(e.target.value)}
              ...
              style={{ width: '100%', padding: '11px 14px', borderRadius: 10, border: '1.5px solid #e0e6ee',
                fontSize: 14, color: '#1a1a1a', marginBottom: 16, boxSizing: 'border-box' }} />
        ...
        {step === 'type' && (
          <>
            ...
            <textarea value={content} onChange={e => setContent(e.target.value)}
              ...
              rows={10}
              style={{ width: '100%', padding: '12px 14px', borderRadius: 10, border: '1.5px solid #e0e6ee',
                fontSize: 13, color: '#1a1a1a', resize: 'vertical', fontFamily: 'inherit',
                lineHeight: 1.6, boxSizing: 'border-box', marginBottom: 16 }} />

            <button onClick={handleSave} disabled={!title.trim() || !content.trim()}
              style={{
                width: '100%', padding: '14px 0', borderRadius: 12, border: 'none',
                background: title.trim() && content.trim() ? BRAND.navy : '#ccd5de',
        ...
        {step === 'upload' && (
          <>
            ...
            <div style={{
              ...
              <p style={{ fontSize: 14, fontWeight: 600, color: BRAND.navy, marginBottom: 4 }}>
    ...
    <div style={{ display: 'flex', flexDirection: 'column', height: '100%' }}>
      ...
      <div style={{
        ...
        <button onClick={() => setConfirmDelete(true)} style={{
          ...
          alignItems: 'center', gap: 4, color: '#fca5a5', fontSize: 12, fontWeight: 600,
        }}>
      ...
      <div style={{ flex: 1, overflowY: 'auto', padding: '20px 16px' }}>
        ...
        {/* Full content */}
        <div style={{
          background: '#fff', borderRadius: 14, padding: '16px', border: '1px solid #e8edf2',
        ...
        <p style={{ fontSize: 11, color: '#bbb', textAlign: 'center', marginTop: 8 }}>
          Coming soon — will open the MatMind AI channel with this entry in context
        </p>
      </div>
        ...
        <div style={{
          ...
          <div style={{
            ...
            <p style={{ fontSize: 13, color: '#888', marginBottom: 24 }}>
              "{entry.title}" will be removed from the Knowledge Base. This cannot be undone.
            </p>
            <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 10 }}>
...
function EntryCard({ entry, onClick }) {
  ...
  return (
    <button onClick={onClick} style={{
      width: '100%', textAlign: 'left', background: '#fff', border: '1px solid #e8edf2',
      borderRadius: 14, padding: '14px', cursor: 'pointer', display: 'block',
      ...
      <p style={{ fontSize: 12, color: '#777', lineHeight: 1.5, marginBottom: entry.fileName ? 8 : 0 }}>
        {preview}
      </p>
      {entry.fileName && (
...
export default function KnowledgeBaseTab({ isCoach = true }) {
  ...
  return (
    <div style={{ display: 'flex', flexDirection: 'column', height: '100%' }}>
          ...
          {CATEGORIES.map(cat => {
            ...
            return (
              <button key={cat.id} onClick={() => setFilter(cat.id)} style={{
                ...
                color: isActive ? cat.color : '#aab4c0',
                whiteSpace: 'nowrap',
              }}>
      ...
      {/* Add Entry sheet */}
      {showAdd && <AddEntrySheet onClose={() => setShowAdd(false)} onSave={handleSave} />}
    </div>
  );
}
